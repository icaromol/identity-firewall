...
- **encrypted-at-rest Vault, split across independent tiers** — private keys, personal data, and credentials are never persisted in plaintext, so a partial compromise (e.g. read access to extension storage without code execution) does not itself disclose Vault contents. Per [ADR-015](adr/ADR-015-three-tier-vault-storage.md), the vault is not one encrypted blob: an always-decrypted index holds Root Identity plus per-origin *metadata only* (no credential/alias values), and each origin's actual secrets live in their own separately-encrypted payload, decrypted only when that specific origin is accessed. This bounds the blast radius of a routine operation (checking one site's credentials no longer decrypts every other site's secrets in the process) without weakening the encrypted-at-rest guarantee itself — see the ADR for what this does and does not defend against;
