...
#### M4/M5 — `/code-review` findings, all fixed

- **Cross-origin data leak (the serious one)**: `handleSubmitFieldDecisions` trusted the `origin`/`tabId` the popup sent without re-checking them against anything. If the tab navigated to a different site while the popup stayed open (a redirect, or the user following a link) and the user then clicked Submit, the handler would resolve `PersonalData` for the *original* origin and relay it via `browser.tabs.sendMessage` straight into whatever page the tab now showed — a direct violation of this project's per-site isolation principle. **Fixed**: `handleSubmitFieldDecisions` now calls `browser.tabs.get(tabId)` first and refuses (throws) unless the tab's *current* url still matches the claimed origin. This reuses the exact signal `activeTab`'s revocation-on-navigation already provides (M4's as-built notes) — once the tab navigates, `tab.url` comes back stripped, and the check fails closed rather than silently trusting stale state.
- **`shared/fieldKey.ts`'s key could collide within a single form**: `field.name ?? field.id ?? '#index'` is not unique when two fields legitimately share a `name` (e.g. separate "Billing address"/"Shipping address" fieldsets both using `name="address"`, a real-world pattern) — the decisions map and the resolved autofill values would silently overwrite/duplicate between them. **Fixed**: the key is now always index-prefixed (`` `${index}:${name-or-id-or-'field'}` ``), guaranteeing uniqueness by construction regardless of what the page names its fields; the name/id suffix is now purely cosmetic for error messages.
- **`stores/firewall.store.ts`'s `submitError` was a single store-wide field**: with more than one pending form on a page, a failure submitting form 0 would render as an error under form 1's card too, even though form 1 was never submitted. **Fixed**: renamed to `submitErrors: Record<number, string>`, keyed by `formIndex`.
- Not fixed, an accepted limitation: `content/autofill.ts` re-identifies fields purely by position at fill time, with no check that the DOM still matches what was classified at `FORM_DETECTED` time. A page that dynamically inserts/removes an unnamed field between detection and approval could shift the positional fallback onto the wrong element. Judged low-likelihood (most real forms have named fields; this path only matters when both `name` and `id` are absent) and would need a larger redesign (carrying per-field structural metadata through to autofill time for a defensive check) to close properly — left as a known gap rather than a quick patch.
