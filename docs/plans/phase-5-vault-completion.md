...
Built as planned, with one real architectural consequence discovered while wiring it in, and the automated `/code-review` pass replaced with a manual self-review this one time (the session's rate limit reset window meant every finder agent failed on the API call itself, not on anything about the diff):

- **`computeAutoApply` stops being strictly "no I/O."** A Synthetic value's derivation needs Web Crypto's `crypto.subtle.deriveBits`, which has no synchronous form at all — there is no way to keep this function synchronous and still derive deterministically via the one cryptography API this project is allowed to use (ADR-003). `computeAutoApply` and `generateResponseValue` are both `async` now; `background/firewall/handler.ts`'s manual `handleSubmitFieldDecisions` path converts its own `.forEach()` to a `for...of` loop for the same reason — `.forEach()` never awaits an async callback, which would have let the function return before every field's value actually finished resolving.
- **`rootSecret` threads through the Policy Engine for the first time**, extracted from the exact same already-fetched vault index `tryLoadAutoApplyInputs()`/`loadPolicyContext()` already read for `aliasProviderConfig`/`policies` — no new storage read anywhere in this milestone.
- **The ADR's own `info` string gained a `synthetic:` prefix not in the original pseudocode**, added specifically for domain separation: without it, this derivation and ADR-010's Service Identity derivation would share the same `info` (the origin alone) for any field type that happened to collide, deriving identical bytes for two completely different purposes from the same root secret. ADR-016 updated with the as-built specifics rather than left describing something slightly different from what actually shipped.
- **Manual self-review covered**: every production call site of the three newly-async functions is correctly awaited (verified by grep, not just by the type-checker passing); the `.forEach()`→`for...of` conversion correctly preserves `forEach`'s `(field, index)` argument order against `.entries()`'s `[index, field]` yield order (an easy index/value swap to introduce silently); no derived token or raw `rootSecret` ever crosses a message boundary (`AUTOFILL_FIELDS`'s payload carries only the final resolved string); and the extra `origin`/`rootSecret` parameters cost nothing for the `real`/`nonsense`/`deny` branches, which never touch them.
