...
- The derivation scheme (root → per-origin identity) is one of the project's genuinely custom pieces; it is designed with reference to prior art (e.g. Attestto's pairwise `did:jwk` per origin) but implemented independently, since it's central to the project's differentiation rather than infrastructure to be reused as-is.
