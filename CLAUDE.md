...
Standing instructions for any Claude Code session working on this repository. Phases 1–6 (extension foundation; local identity vault; the Identity Firewall itself; the Policy Engine and Privacy Ledger; Vault Completion — personal data UI, password generation, credential capture/save/fill, deterministic synthetic values; Extension Dashboard — a standalone Options page with 3 tabs: all-sites "who knows what about me", Personal Data, Backup & Recovery) are implemented and merged; Phase 7 (Vault Session Security & Biometric Authorization) is in progress — Part A (auto-lock via `chrome.idle`, a new "Configuration" tab, credential auto-save wiring, and per-field default-policy dropdowns finally giving the Policy Engine's global rules a UI) is complete and manually verified in a real browser; Part B (biometrics) has not started. Folded into one phase rather than triggering a third full roadmap renumbering — see `docs/roadmap.md` and `docs/plans/` for current status. Phase 6 was narrowed directly with the user from its originally-scoped 4 killer features down to just 7.1 (extended to all sites); Revoke Identity (7.2), Privacy Score (7.3), and the disclosure-reduction metric (7.4) were dropped from it entirely and are currently unscheduled. The roadmap was restructured to insert Phase 5 (Vault Completion), Phase 6 (Extension Dashboard), and Phase 8 (In-Page Autofill & Auto-Login) after the original Phase 4; what were Phases 5–12 are now Phases 7, 9–15 respectively — always check `docs/roadmap.md`'s own phase numbers rather than assuming an older numbering from memory. Three honestly-documented real-world limitations are carried forward, not fixed: dynamically-rendered forms (e.g. Figma's signup SPA) aren't detected at all, multi-step wizard forms can silently fill a hidden/inactive step (both from Phase 4), and Fill fails against a login form rendered by a third-party auth widget after the page's one-shot detection pass (Phase 5, e.g. Clerk-based login forms) — all three are Phase 9 ("Detection of dynamically rendered pages") territory, not any earlier phase's. Read this file fully before making architectural decisions or starting implementation.
...
- `docs/adr/` — 19 Architecture Decision Records (ADR-001 through ADR-019) covering local-first, browser-extension distribution, Web Crypto, pairwise service identities, biometrics-as-unlock, no-blockchain, no-server-dependency, deferred DID/VC/SDK, the personal-project-not-startup pivot, the finalized HKDF-based identity derivation function, the metadata-only WebAuthn integration mode, the vault unlock key's PRF-primary/Argon2id-passphrase-fallback derivation, the three-key hierarchy (VaultUnlockKey/RootSecret/BackupExportKey), the empirically-verified Ed25519 seed derivation for Service Identity keys, the three-tier vault storage split (index/personal-data/per-site payload) that scopes decryption to only what a given operation actually needs, the deterministic per-site derivation of fake Synthetic personal-data values, the `chrome.idle`-based auto-lock mechanism (over `chrome.alarms`), the `background/settings/` module's separably-extractable boundary, and the local dev log's vault-reuse-not-a-second-password access-control design.
...
- `docs/plans/` — detailed, phase-by-phase execution plans, one level more concrete than `docs/roadmap.md`'s phase objectives. Currently: `phase-1-extension-foundation.md` (milestones, directory tree, acceptance checklist for Phase 1 — complete), `phase-2-local-identity-vault.md` (milestones M1–M9 for the vault's crypto/storage/key-hierarchy design — complete), `phase-2-vault-tiering-refactor.md` (the sequenced, copy-paste-prompt execution plan for ADR-015's three-tier storage retrofit across M1–M7's already-shipped code — complete), `phase-3-identity-firewall.md` (milestones M1–M6 for the field classifier, response generator, approval UI, and autofill round-trip — complete), `phase-4-privacy-ledger-policy-engine.md` (milestones M1–M7 for the Policy Engine's resolution logic and the Privacy Ledger — complete), `phase-5-vault-completion.md` (milestones M1–M7 for the personal data UI, password generator, credential capture/save/fill, and deterministic Synthetic values — complete), `phase-6-extension-dashboard.md` (milestones M1–M5 for the standalone Options page: an all-sites "who knows what about me" tab, Personal Data, and Backup & Recovery, relocated out of the popup — complete), `autolock-and-configuration.md` (Phase 7 Part A: `chrome.idle`-based auto-lock, the new `background/settings/` module, the Configuration tab, and the Personal Data per-field policy dropdowns — complete; Phase 7 Part B, biometrics, gets its own `phase-7-biometric-authorization.md` when that part starts), `local-dev-log.md` (a persisted, vault-gated dev log — complete; not tied to any roadmap phase, see ADR-019), and `password-generator-popup-ui.md` (a standalone "Generate password" popup section, independent of any on-page form — complete; closes a residual Phase 5 gap, the generator primitive existed since Phase 5 M2 but was never wired to any UI until now). Add a new file here, following the same pattern, before starting implementation on each subsequent phase.
