...
- `docs/adr/` — 13 Architecture Decision Records (ADR-001 through ADR-013) covering local-first, browser-extension distribution, Web Crypto, pairwise service identities, biometrics-as-unlock, no-blockchain, no-server-dependency, deferred DID/VC/SDK, the personal-project-not-startup pivot, the finalized HKDF-based identity derivation function, the metadata-only WebAuthn integration mode, the vault unlock key's PRF-primary/Argon2id-passphrase-fallback derivation, and the three-key hierarchy (VaultUnlockKey/RootSecret/BackupExportKey). ADR-014 (Ed25519 vs. ECDSA for Service Identity keys) is planned but deliberately deferred to Phase 2 M5, once Ed25519 seed-import behavior is verified empirically.
...
- `docs/plans/` — detailed, phase-by-phase execution plans, one level more concrete than `docs/roadmap.md`'s phase objectives. Currently: `phase-1-extension-foundation.md` (milestones, directory tree, acceptance checklist for Phase 1 — complete) and `phase-2-local-identity-vault.md` (milestones M1–M9 for the vault's crypto/storage/key-hierarchy design — M1–M6 implemented, M7–M9 open). Add a new file here, following the same pattern, before starting implementation on each subsequent phase.
