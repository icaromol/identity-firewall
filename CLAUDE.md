...
Standing instructions for any Claude Code session working on this repository. Phases 1–3 (extension foundation; local identity vault; the Identity Firewall itself — field classification, approval UI, autofill) are implemented and merged; Phase 4 (Privacy Ledger + Policy Engine) is in progress — see `docs/roadmap.md` and `docs/plans/` for current status. Read this file fully before making architectural decisions or starting implementation.
...
- `docs/adr/` — 15 Architecture Decision Records (ADR-001 through ADR-015) covering local-first, browser-extension distribution, Web Crypto, pairwise service identities, biometrics-as-unlock, no-blockchain, no-server-dependency, deferred DID/VC/SDK, the personal-project-not-startup pivot, the finalized HKDF-based identity derivation function, the metadata-only WebAuthn integration mode, the vault unlock key's PRF-primary/Argon2id-passphrase-fallback derivation, the three-key hierarchy (VaultUnlockKey/RootSecret/BackupExportKey), the empirically-verified Ed25519 seed derivation for Service Identity keys, and the three-tier vault storage split (index/personal-data/per-site payload) that scopes decryption to only what a given operation actually needs.
