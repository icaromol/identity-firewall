...
- `docs/plans/` — detailed, phase-by-phase execution plans, one level more concrete than `docs/roadmap.md`'s phase objectives. Currently: `phase-1-extension-foundation.md` (milestones, directory tree, acceptance checklist for Phase 1 — complete) and `phase-2-local-identity-vault.md` (milestones M1–M9 for the vault's crypto/storage/key-hierarchy design — not yet implemented). Add a new file here, following the same pattern, before starting implementation on each subsequent phase.
