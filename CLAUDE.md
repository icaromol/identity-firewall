...
- `docs/plans/` — detailed, phase-by-phase execution plans, one level more concrete than `docs/roadmap.md`'s phase objectives. Currently: `phase-1-extension-foundation.md` (milestones, directory tree, acceptance checklist for Phase 1 — complete), `phase-2-local-identity-vault.md` (milestones M1–M9 for the vault's crypto/storage/key-hierarchy design — M1–M7 implemented, M8–M9 open), and `phase-2-vault-tiering-refactor.md` (the sequenced, copy-paste-prompt execution plan for ADR-015's three-tier storage retrofit across M1–M7's already-shipped code — not yet started). Add a new file here, following the same pattern, before starting implementation on each subsequent phase.
